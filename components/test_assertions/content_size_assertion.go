...
func (assertion ContentSizeAssertion) Check(t *testing.T, component components.Component) {
	...
	require.Equal(t, assertion.ExpectedMinWidth, minWidth)
	require.Equal(t, assertion.ExpectedMaxWidth, maxWidth)
	require.Equal(t, assertion.ExpectedMinHeight, minHeight)
	require.Equal(t, assertion.ExpectedMaxHeight, maxHeight)
}

// Helper to create multiple content size assertions
func GetContentSizeAssertions(dimensions ...int) []ContentSizeAssertion {
	if len(dimensions)%4 != 0 {
		panic("Must provide dimensions in pairs of (minWidth, maxWidth, minHeight, maxHeight)")
	}

	result := make([]ContentSizeAssertion, len(dimensions)/2)
